...
cols.with.ab.zeros = function(my.mat, a=0, b=Inf) {
  # zeros.per.column should be counting the number of zero values per column in the given matrix, but colSums(mat != 0) produces the number of non-zero values per column. Changed to colSums(mat == 0).
  zeros.per.column = colSums(my.mat == 0)
  # i.to.keep signifies which columns to keep. cols.with.ab.zeroes(mat, a=1, b=1) returns nothing, although it should return the middle column. Something is wrong with our i.to.keep definition.
  i.to.keep = a <= zeros.per.column && zeros.per.column >= b
  # columns are in second position when selecting matrix elements, so comma should be in front. Use as.matrix to keep it in matrix form if there is only one column.
...
list.extractor = function(my.list, i.to.keep=NULL, i.to.remove=NULL) {
  ...
  # switch position of the 'if' statements so that i.to.keep overrides i.to.remove rather than vice-versa.
  return(L)
}
